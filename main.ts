...
export default class HotkeysChordPlugin extends Plugin {
	...
	async onload() {
		...
		data.hotkeys = data.hotkeys.map((chord: any) => new Chord({
			command: chord.command,
			sequence: chord.sequence.map((hotkey: any) => new HotKey(hotkey)),
		}));
...
function ChordCapture(cb: (seq: HotKey[], final: boolean) => void) {
	var sequence: HotKey[] = [];
	var keydownhandler: (ev: KeyboardEvent) => void = event => {
